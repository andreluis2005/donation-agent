...
export async function POST(request: Request) {
	try {
		...
		console.log(
			"Enviando comando para o agente:",
			command,
			"Doar ao dev:",
			donateToDev,
		);
		...
		if (!match) {
			return NextResponse.json(
				{
					error:
						"Invalid command format. Use: Donate [amount] [currency] to [cause/address].",
				},
				{ status: 400 },
			);
		}
		...
		if (/^0x[a-fA-F0-9]{40}$/.test(target)) {
			toAddress = target as `0x${string}`; // Usar endereço personalizado
		...
		try {
			...
			if (result.value && result.toAddress && result.currency) {
				if (
					...
					return NextResponse.json(
						{ error: "Mismatch in address or currency returned by model." },
						{ status: 400 },
					);
				}
				return NextResponse.json({
